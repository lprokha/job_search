...
public class ProfilePageController {
    ...
    private void fillProfileModel(User currentUser, Model model) {
        ...
        if (currentUser.getAccountType() == AccountType.APPLICANT) {
            ...
            model.addAttribute("responsesCount", respondedApplicantService.countByApplicantId(currentUser.getId()));
        } else {
            List<Vacancy> vacancies = vacancyService.getByAuthorId(currentUser.getId());

            Map<String, Long> vacancyResponseCounts = new LinkedHashMap<>();

            for (Vacancy vacancy : vacancies) {
                vacancyResponseCounts.put(
                        vacancy.getId().toString(),
                        respondedApplicantService.countByVacancyId(vacancy.getId())
                );
            }

            model.addAttribute("vacancies", vacancies);
            model.addAttribute("vacancyUpdateTimes", buildVacancyUpdateTimeMap(vacancies));
            model.addAttribute("vacancyResponseCounts", vacancyResponseCounts);
            model.addAttribute("responsesCount", respondedApplicantService.countByEmployerId(currentUser.getId()));
        }
    }
